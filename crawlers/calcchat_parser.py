...
def main(chapters_to_download):
	...
	chapters = BeautifulSoup(requests.get(CHAPTERS_URL).content, "html.parser").find_all("chapter")
...
def download_exercise(exercise_id, section):
	url = EXERCISE_DOWNLOAD_URL.format(section_pre=section.pre, ex_id=format_as_hundread(exercise_id))
